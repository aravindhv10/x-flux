...
def create_embeddings(list_path_images, list_path_captions):
    list_path_ae_output = list(
        remove_extension(path_input=i) + '.safetensors'
        for i in list_path_images)

    list_path_text_embedding_output = list(
        remove_extension(path_input=i) + '.safetensors'
        for i in list_path_captions)
...
def get_list_captions(list_path_image_files):
    return list(
        os.path.join(os.path.dirname(i), 'caption.txt')
        for i in list_path_image_files)


def create_embeddings_in_docker():

    list_path_images = get_main_image_files('/data/input')
    list_path_captions = get_list_captions(list_path_images)

    create_embeddings(list_path_images, list_path_captions)
