...
def get_models_dit(name: str, device, offload: bool, is_schnell: bool):
    model = load_flow_model2(name, device="cpu")
    return model
...
def main():
    ...
    dit = get_models_dit(name=args.model_name,
                         device=accelerator.device,
                         offload=False,
                         is_schnell=is_schnell)
    ...
    print(
        sum([p.numel()
             for p in controlnet.parameters() if p.requires_grad]) / 1000000,
        'parameters')
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                if accelerator.sync_gradients:
                    accelerator.clip_grad_norm_(controlnet.parameters(),
                                                args.max_grad_norm)
                optimizer.step()
                lr_scheduler.step()
                optimizer.zero_grad()
