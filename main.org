...
#+begin_src sh :shebang #!/bin/sh :results output 
  git commit -m 'Started work on controlnet + lora unified trainer'
  ssh-add "${HOME}/.ssh/github10"
  git push --set-upstream origin aravind_prodigy_dataset
#+end_src

#+RESULTS:
: [aravind_prodigy_dataset ce0c887] Started work on controlnet + lora unified trainer
:  5 files changed, 605 insertions(+), 33 deletions(-)
:  create mode 100644 train_flux_lora_controlnet_deepspeed_cached.py
