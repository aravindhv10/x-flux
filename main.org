...
#+begin_src sh :shebang #!/bin/sh :results output 
  git commit -m 'Wrote code to generate latent for new type dataset'
  ssh-add "${HOME}/.ssh/github10"
  git push --set-upstream origin aravind_prodigy_dataset
#+end_src

#+RESULTS:
: [aravind_prodigy_dataset 7734cec] Wrote code to generate latent for new type dataset
:  3 files changed, 228 insertions(+), 8 deletions(-)
:  create mode 100644 make_latent_2.py
